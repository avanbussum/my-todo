...
export default function Home() {
  ...
  const addPatient = async e => {
    ...
    let patient = {
      patientText: firstName, lastName,
      isDeleted: false
    }
    ...
    try {
      ...
      if (ethereum) {
        ...
        PatientContract.addPatient(patient.patientText, patient.isDeleted)
        .then(res => {
  ...
  return (
    <div className='bg-[#97b5fe] h-screen w-screen flex justify-center py-6'>
      {!isUserLoggedIn ? <ConnectWalletButton connectWallet={connectWallet}/> :
        'is this the correct network?' ? <TodoList patients={patients} firstName={firstName} setInput1={setInput1} lastName={lastName} setlastName={setlastName} addPatient={addPatient} deletePatient={deletePatient}/> : 
      <WrongNetworkMessage />}
    </div>
  )
}
