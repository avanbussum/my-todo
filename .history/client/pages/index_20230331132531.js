  ...
  const getEvents = async () => {
    try {
      if (ethereum) {
        ...
        const filterFromAddress1 = PatientContract.filters.AddPatient(address1)

        const logs = await PatientContract.queryFilter(filterFromAddress1)
        console.log(logs)
        const inter = new ethers.Interface(ABI)
        
        const parsedLogs = logs.map(log => {
          return inter.parseLog({ data: log.data, topics: log.topics });
        })
        setEventLog(parsedLogs)
        contract.on("AddPatient", (recipient, patientID) => {
          console.log("New AddPatietn event with the arguments:")
          console.log(ecipient, patientID)
      })
