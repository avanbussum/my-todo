...
export default function Home() {
  ...
  const [eventLog, setEventLog] = useState([{accesor:'', patientId: 0}])
  ...
  const getEvents = async () => {
    try {
      if (ethereum) {
        ...
        const parsedLogs [{accesor:'', patientId: 0}] = events.map(log => {
           return {inter.parseLog(log).args, inter.parseLog(log).args}
        })
