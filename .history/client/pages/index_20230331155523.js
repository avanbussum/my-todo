...
export default function Home() {
  ...
  const [eventLog, setEventLog] = useState([{accesor:'', patientId:''}])
  ...
  const getEvents = async () => {
    try {
      if (ethereum) {
        ...
        //   await setEventLog([...eventLog, {accesor: parsed.args[0], patientId: parsed.args[1]}])
        // }
        ...
        parsedLogs.map(arg => async () {
          await setEventLog([...eventLog, {accesor: arg[0] , patientId: arg[1]}])
        })
       console.log('OFFICIAL logs:', eventLog)
